...
//        V_DrawShadowedPatch(xpos + 6 - patch->width / 2 - 12, y, patch);	// NOT FOR THE WII
        V_DrawShadowedPatch(xpos + 6 - SHORT(patch->width) / 2 - 12, y, patch);	// FIX FOR THE WII
    }
    if (digits > 2)
    {
        if (realdigits > 2)
...
//    V_DrawShadowedPatch(xpos + 6 - patch->width / 2, y, patch);		// NOT FOR THE WII
    ...
    {
        patch = FontBNegative;
//        V_DrawShadowedPatch(xpos + 6 - patch->width / 2 - 12 * (realdigits),	// NOT FOR THE WII
//                            y, patch);
        V_DrawShadowedPatch(xpos + 6 - SHORT(patch->width) / 2 - 12 * (realdigits), // FIX FOR THE WII
                            y, patch);
    }
}
...
//            x += p->width - 1;						// NOT FOR THE WII
            x += SHORT(p->width) - 1;						// FIX FOR THE WII
        }
    }
}
