...
//static int finit_height = SCREENHEIGHT - 42;				// CHANGED FOR HIRES
static int finit_height = SCREENHEIGHT - (42 << hires);			// CHANGED FOR HIRES
...
/*static*/extern int followplayer/* = 1*/;    // specifies whether to follow the player around
extern int drawgrid;
...
/*
  mapxstart += MTOF(m_paninc.x+FRACUNIT/2);			// CHANGED FOR HIRES
  mapystart -= MTOF(m_paninc.y+FRACUNIT/2);			// CHANGED FOR HIRES
  if(mapxstart >= finit_width)					// CHANGED FOR HIRES
		mapxstart -= finit_width;			// CHANGED FOR HIRES
  if(mapxstart < 0)						// CHANGED FOR HIRES
		mapxstart += finit_width;			// CHANGED FOR HIRES
  if(mapystart >= finit_height)					// CHANGED FOR HIRES
		mapystart -= finit_height;			// CHANGED FOR HIRES
  if(mapystart < 0)						// CHANGED FOR HIRES
		mapystart += finit_height;			// CHANGED FOR HIRES
*/
    ...
    mapxstart += MTOF(m_paninc.x+FRACUNIT/2);			// CHANGED FOR HIRES
    mapystart -= MTOF(m_paninc.y+FRACUNIT/2);			// CHANGED FOR HIRES
    if(mapxstart >= (finit_width >> hires))			// CHANGED FOR HIRES
        mapxstart -= (finit_width >> hires);			// CHANGED FOR HIRES
    if(mapxstart < 0)						// CHANGED FOR HIRES
        mapxstart += (finit_width >> hires);			// CHANGED FOR HIRES
    if(mapystart >= (finit_height >> hires))			// CHANGED FOR HIRES
        mapystart -= (finit_height >> hires);			// CHANGED FOR HIRES
    if(mapystart < 0)						// CHANGED FOR HIRES
        mapystart += (finit_height >> hires);			// CHANGED FOR HIRES
...
void AM_Start(void)
{
    if(players[consoleplayer].pendingweapon != wp_gauntlets)
    {
...
( event_t*	ev )
{
    // THESE LINES ARE COMING STRAIGHT FROM NITR8...
    // ...(WHO BROUGHT THIS PORT / GAME TO THE PSP):
    //
    // THIS FUNCTION (OR MAINLY THE 'OLD' CODE...
    // ...FROM CHOCOLATE) WAS A PAIN IN THE BUTT...
    // ...AS IT'S HARD FOR THE LOW AMOUNT OF KEYS...
    // ...ON THE PSP TO CONTROL AS MUCH AS POSSIBLE!!!
    //
    // SO I TOOK THE CODE FROM PSPHEXEN AND CHANGED...
    // ...A FEW LINES AFTERWARDS (ALSO THE CALL OF...
    // ...THE FUNCTION 'AM_RESPONDER' HAD TO BE...
    // ...MOVED FROM THE FUNCTION 'G_RESPONDER'...
    // ...IN THE FILE 'G_GAME.C' TO THE FUNCTION...
    // ...'D_PROCESSEVENTS' IN THE FILE 'D_MAIN.C')!!!
    int rc;
...
( event_t*  ev )
{
	...
	else if (data->btns_d & WPAD_CLASSIC_BUTTON_ZL)
	{
	    mtof_zoommul = M_ZOOMIN;
	    ftom_zoommul = M_ZOOMOUT;
	}
...
//                      dmapx=0;  //goes into the automap.
        ...
        mapystart += dmapy >> 1;
/*
        while (mapxstart >= finit_width)				// CHANGED FOR HIRES
            mapxstart -= finit_width;					// CHANGED FOR HIRES
*/
        while (mapxstart >= (finit_width >> hires))			// CHANGED FOR HIRES
            mapxstart -= (finit_width >> hires);			// CHANGED FOR HIRES
        while (mapxstart < 0)
/*
            mapxstart += finit_width;					// CHANGED FOR HIRES
        while (mapystart >= finit_height)				// CHANGED FOR HIRES
            mapystart -= finit_height;					// CHANGED FOR HIRES
*/
            mapxstart += (finit_width >> hires);			// CHANGED FOR HIRES
        while (mapystart >= (finit_height >> hires))			// CHANGED FOR HIRES
            mapystart -= (finit_height >> hires);			// CHANGED FOR HIRES
        while (mapystart < 0)
//            mapystart += finit_height;				// CHANGED FOR HIRES
            mapystart += (finit_height >> hires);			// CHANGED FOR HIRES
...
//    j = mapystart * finit_width;					// CHANGED FOR HIRES
    j = (mapystart & ~hires) * (finit_width >> hires);			// CHANGED FOR HIRES
    for (i = 0; i < finit_height; i++)
    {
        ...
        memcpy(I_VideoBuffer + i * finit_width + finit_width - mapxstart,
               maplump + j, mapxstart);
        j += finit_width;
//        if (j >= finit_height * finit_width)				// CHANGED FOR HIRES
        if (j >= (finit_height >> hires) * (finit_width >> hires))	// CHANGED FOR HIRES
            j = 0;
    }

//       memcpy(screens[0], maplump, finit_width*finit_height);
...
//      }
    ...
    {
        oldyy++;
//        oldyyshifted += 320;				// CHANGED FOR HIRES
        oldyyshifted += (320 << hires);			// CHANGED FOR HIRES
    }
    else if (yy == oldyy - 1)
    {
        oldyy--;
//        oldyyshifted -= 320;				// CHANGED FOR HIRES
    ...
    {
        oldyy = yy;
//        oldyyshifted = yy * 320;			// CHANGED FOR HIRES
        oldyyshifted = yy * (320 << hires);		// CHANGED FOR HIRES
    }
...
void AM_rotate(fixed_t* x,  fixed_t* y, angle_t a, fixed_t xorig, fixed_t yorig)
{
  fixed_t tmpx;
        ...
        while (t)
        {
  	    ...
  	    if (automapactive & am_rotate)
            ...
            AM_drawLineCharacter(thintriangle_guy, NUMTHINTRIANGLEGUYLINES,
                                 /*16 << FRACBITS*/radius, t->angle, colors + lightlev,
                                 /*t->*/x, /*t->*/y);
            t = t->snext;
        }
    }
...
*/
        MN_DrTextA(DEH_String(hud_monsecstr), -20, 15);
    }
}
