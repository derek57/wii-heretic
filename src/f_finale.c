...
//        cx += w->width;				// NO DEFINITELY NOT ON THE WII
        cx += SHORT(w->width);				// FIX FOR THE WII
    }
...
void F_DemonScroll(void)
{
    ...
    if (finalecount < 70)
    {
//        memcpy(I_VideoBuffer, p1, SCREENHEIGHT * SCREENWIDTH);		// CHANGED FOR HIRES
        V_CopyScaledBuffer(I_VideoBuffer, p1, ORIGHEIGHT * ORIGWIDTH);		// CHANGED FOR HIRES
    ...
    {
/*
        memcpy(I_VideoBuffer, p2 + SCREENHEIGHT * SCREENWIDTH - yval, yval);	// CHANGED FOR HIRES
        memcpy(I_VideoBuffer + yval, p1, SCREENHEIGHT * SCREENWIDTH - yval);	// CHANGED FOR HIRES
        yval += SCREENWIDTH;							// CHANGED FOR HIRES
*/
        V_CopyScaledBuffer(I_VideoBuffer, p2 + ORIGHEIGHT * ORIGWIDTH - yval, yval);	// CHANGED FOR HIRES
        V_CopyScaledBuffer(I_VideoBuffer + (yval << (2 * hires)), p1, ORIGHEIGHT * ORIGWIDTH - yval);	// CHANGED FOR HIRES
        yval += ORIGWIDTH;
        nextscroll = finalecount + 3;
    }
    else
    {                           //else, we'll just sit here and wait, for now
//        memcpy(I_VideoBuffer, p2, SCREENWIDTH * SCREENHEIGHT);		// CHANGED FOR HIRES
        V_CopyScaledBuffer(I_VideoBuffer, p2, ORIGWIDTH * ORIGHEIGHT);		// CHANGED FOR HIRES
    }
}
...
//                I_ResetScaleTables(palette);
		...
		V_DrawPatch(0, 0, W_CacheLumpName(DEH_String("TITLE"), PU_CACHE));
	    else
...
void F_Drawer(void)
{
    ...
    else
    {
        switch (gameepisode)
        {
			...
			V_DrawPatch(0, 0, W_CacheLumpName(DEH_String("ORDER"), PU_CACHE));
                }
                else
                {
