...
{
    for(input_point = inputtext; MN_TextAWidth(input_point) > 320 - 20; input_point++);
}
...
// draw the console
// FIXED TO MATCH HERETIC / HEXEN FONT (FONTA*)
...
*/  
    ...
    y = current_height - ((c_showprompt && message_pos == message_last) ? 10 : 0);
...
//      V_WriteText(messages[count], 0, y);
    ...
    if(current_height > 10 && c_showprompt && message_pos == message_last)
    {
	char tempstr[LINELENGTH];
	char tempstr2[LINELENGTH];
	char *underline;
	char *version = "                                                    V1.0";
	...
	if(message_pos == message_last)
	{
	    sprintf(tempstr2, "%s", inputprompt);
	    sprintf(tempstr, "%s%s%s", input_point, underline, version);
	}
/*
...
*/
	MN_DrTextA(tempstr2, 0, current_height - 10);

	if(leveltime & 16)
	    MN_DrTextA(tempstr, 11, current_height - 10);
	else
	    MN_DrTextA(tempstr, 12, current_height - 10);
    }
}
...
static void C_AddMessage(char *s)
{
    ...
    if(MN_TextAWidth(messages[message_last]) > SCREENWIDTH - 9 ||
...
static void C_AdjustLineBreaks(char *str)
{
	...
	if(MN_TextAWidth(str) + MN_TextAWidth(messages[message_last]) > SCREENWIDTH - 9
