...
void SB_Init(void)
{
    ...
    PatchXHAIR = W_CacheLumpName("XHAIR", PU_STATIC);
/*
    if (deathmatch)
    {
...
void SB_Ticker(void)
{
    ...
    if (leveltime & 1 && !MenuActive)			// ADDED MenuActive CONDITION HERE AS A FIX FOR
    {							// WIIHERETIC'S MAIN MENU REFRESHING ACTIVITIES
...
static void ShadeLine(int x, int y, int height, int shade)
{
    ...
    x <<= hires;					// ADDED FOR HIRES
    y <<= hires;					// ADDED FOR HIRES
    height <<= hires;					// ADDED FOR HIRES
    ...
    while (height--)
    {
        if (hires)					// ADDED FOR HIRES
            *(dest + 1) = *(shades + *dest);		// ADDED FOR HIRES
